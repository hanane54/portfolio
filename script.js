...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function fetchGitHubData() {
    ...
    try {
      const [userResponse, reposResponse] = await Promise.all([
        fetch(endpoints.user, fetchOptions),
        fetch(endpoints.repos, fetchOptions)
      ]);
  ...
  async function loadGitHubActivity() {
    ...
    try {
      const response = await fetch(endpoints.activity, fetchOptions);
      if (response.ok) {
        ...
        // Count commits and show recent activity
        const recentCommits = activityData
          .filter(event => event.type === 'PushEvent')
          .reduce((sum, event) => sum + (event.payload?.commits?.length || 0), 0);
        
        document.getElementById('githubCommits').textContent = recentCommits;
      ...
      document.getElementById('githubCommits').textContent = 'N/A';
    }
  }
